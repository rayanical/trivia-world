...
export default function SoloGamePage() {
    ...
    useEffect(() => {
        fetch('https://opentdb.com/api_category.php')
            .then((res) => res.json())
            .then((data) => {
                // FIX: Format category names here
                const formattedCategories = data.trivia_categories.map((cat: Category) => ({
                    ...cat,
                    // Remove any prefix like "Entertainment: " or "Science: "
                    name: cat.name.replace(/^(Entertainment:|Science:|History:|Art:|Politics:)\s*/, ''),
                }));
    ...
    const fetchQuestions = useCallback(async () => {
        setIsLoading(true);
        let apiUrl = `https://opentdb.com/api.php?amount=10&type=multiple&category=${selectedCategory}`;
        ...
        const formattedQuestions = data.results.map((q: Question) => ({
            // Use the Question type here
            ...q,
            question: he.decode(q.question),
            correct_answer: he.decode(q.correct_answer),
            all_answers: [...q.incorrect_answers.map((a: string) => he.decode(a)), he.decode(q.correct_answer)].sort(() => Math.random() - 0.5),
        }));
    ...
    if (!gameStarted) {
        ...
        return (
            <div className="flex h-screen flex-col items-center justify-center bg-[#101710] p-4 text-white">
                <h1 className="text-4xl font-bold mb-8">Setup Your Solo Game</h1>
